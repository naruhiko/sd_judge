...
print('\n', df1_name, ': W=', df1_norm_result[0], ', p=',
      df1_norm_result[1], ',  jud: ', df1_norm_result[2], sep='')

df2_norm_result = is_norm_dist(df2)
print(df2_name, ': W=', df2_norm_result[0], ', p=',
      df2_norm_result[1], ',  jud: ', df2_norm_result[2], sep='')


if is_norm_dist(df1)[1] > user_p and is_norm_dist(df2)[1] > user_p:
    bart_result = variance_bartlett(df1, df2)
    print('Bartlett検定(正規分布を仮定): kai2zero=',
          bart_result[0], ', p=', bart_result[1], ',  jud: ', bart_result[2], sep='')
    ...
    print('Levene検定(非正規分布を仮定): W=',
          leve_result[0], ', p=', leve_result[1], ',  jud: ', leve_result[2], sep='')
    variance = leve_result[1]

    if is_rel == 'y':
        mann_result = mann(df1, df2)
        print('Mann-Whitney検定(独立2群・非正規分布を仮定): W=',
              leve_result[0], ', p=', leve_result[1], ',  jud: ', leve_result[2], sep='')

    elif is_rel == 'n':
        wilc_result = wilcoxon_signed(df1, df2)
        print('Wilcoxon Signed-rank検定(関連2群・非正規分布を仮定): W=',
              wilc_result[0], ', p=', wilc_result[1], ',  jud: ', wilc_result[2], sep='')
